...
export const Buttonss = (props) => {
	...
	return (
		<Button
			style={{ transform: `rotate(${90 * type * -1}deg)`, position: "absolute", top: "3.5vh", right: `${type == 1 ? "-4.5%" : "87%"}`, width: "5.5vh", height: "5.5vh" }}
			onClick={() => {
				if (index + type >= 0 && index + type <= limits) setIndex(index + type);
			}}
		>
			<MyArrow type={type}></MyArrow>
		</Button>
	);
};
...
export const MiniCards = (props) => {
	...
	for (let i = props.start; i < props.start + 3; i++)
		if (props.cards[i]) ret.push(props.cards[i]);
		else ret.push({});

	return (
		<>
			{ret.map((x, i) => (
				<MobileCardSmall key={x.id} bg={x.bg} idx={i} clckevent={props.clckevent} />
			))}
		</>
	);
};
